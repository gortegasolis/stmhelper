labelTopics_df <- function(lab_top_result, 
  ...
  lab_top_result[[word_type]] %>%
    ...
    rename_at(vars(starts_with("V")), 
                  funs(str_replace(., "V", "Word")))-> words
  
  words$Topic <- paste0("Topic ", lab_top_result[["topicnums"]])
  
  if (isTRUE(pivot_wrd) == TRUE){words <- pivot_longer(words, cols = starts_with("Word"), values_to = "Words")}
...
label.models_list <- function(models_list,
  ...
  queries_df <- manyTopics_df(models_list = models_list,
                              texts = texts,
                              n_words = n_words,
                              frexweight = frexweight,
                              n_texts = n_texts,
  ...
  tot_topics <- queries_df %>% select(Topic, K) %>% unique() %>% NROW()
  ...
  for (x in unique(queries_df$code)){
    ...
    paste0("Loop ", n_loop, " of ", tot_loops, ". Total topics are ", tot_topics, ".\n") %>% crayon::blue() %>% cat() # Show current and total loops
    
    if (isTRUE(show_used_labels) == TRUE & n_loop > 1){paste0("Used labels: ", paste0(unique(as.character(labels_df$New_label)), collapse = ", "), "\n") %>% crayon::yellow() %>% cat()}
    
    if(tot_loops == tot_topics & n_loop == 1){cat("The current method for label propagation will not save you time.\n")}
  ...
  if (isTRUE(summarise) == TRUE){
    result <- result %>% select(Topic, K, label_words, New_label, Comment) %>% unique()
  }
