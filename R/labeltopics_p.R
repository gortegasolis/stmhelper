...
labelTopics.p <- function(model,
  ...
  if(word_type == "Highest Prob"){type_num <- 2}
  if(word_type == "FREX"){type_num <- 3}
  if(word_type == "Lift"){type_num <- 4}
  if(word_type == "Score"){type_num <- 5}

  if (is.null(last) == T){ last <- model$settings$dim$K}
  ...
  for(x in first:last){
    ...
    topic_labels <- rbind(topic_labels,
                          temp_labels)
